...
@app.route('/earn_data', methods=['POST'])
def earn_data():
    user = current_user()
    if not user:
        return redirect(url_for('login'))

    # prevent duplicate earnings in one day
    existing = DataEntry.query.filter_by(user_id=user.id, source='earned')\
        .filter(DataEntry.created_at >= datetime.utcnow().date())\
        .first()
    if existing:
        flash("You already earned your data today!", "info")
        return redirect(url_for('dashboard'))

    amount_mb = 914  # example value
    ...
    flash(f"You earned {amount_mb} MB for today!", "success")
    return redirect(url_for('dashboard'))
